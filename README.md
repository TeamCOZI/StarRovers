...
- `CelestialBodyStaticMesh`: 원거리 표현용 Static Mesh Component. Data Asset에서 전달되는 BodyMesh와 Material을 그대로 적용한다. C++에서는 Data Asset에서 전달되는 Static Mesh 에셋 필드를 `CelestialBodyStaticMesh`로 두고, 컴포넌트 포인터는 이름 충돌을 피하기 위해 `CelestialBodyStaticMesh_`로 둔다.
- `CelestialBodyDynamicMesh`: 근거리 표현용 Dynamic Mesh Component. Data Asset에서 전달되는 BodyMesh를 원본으로 삼고 Terrain Generator가 조작한 mesh를 적용한다.
...
Lighting은 화면에서 보이는 천체 크기에 따라 천체별로 두 단계로 나눈다. 작게 보이는 천체는 Star의 `StarPointLight`와 `CelestialBodyStaticMesh`를 사용하고, 크게 보이는 천체는 Level에 배치된 Directional Light와 `CelestialBodyDynamicMesh`를 사용한다. 전환 기준은 `BP_CelestialBody(Self)`의 `Star Rovers|Lighting` 항목에 노출된 `DynamicMeshThreshold`이며, 각 천체의 투영 지름이 카메라 frustum 높이에서 차지하는 비율이 이 값 이상이면 Dynamic Mesh 모드로 전환한다. 기본값은 `0.1`이다. 화면 밖 frustum에 걸치지 않는 천체는 Dynamic Mesh 모드 대상에서 제외한다. `ASRCameraPawn`은 Level의 첫 Directional Light Actor를 찾아 Dynamic Mesh 조명용으로 사용한다. Directional Light 방향은 Dynamic Mesh 모드인 비항성 천체 중 현재 Focused Body를 우선하고, Focused Body가 없으면 화면상 가장 크게 보이는 천체를 향하도록 Primary Star 기준으로 갱신한다. Light Channel, Cast Shadow, Collision, Visibility 기본 옵션은 BP/Level Actor에서 직접 설정하며, C++는 런타임 모드 전환에 필요한 Static/Dynamic Mesh 표시와 Star Point Light/Directional Light 표시 및 방향만 전환한다.

공통 `BP_CelestialBody(Self)`의 `Star Rovers` 항목에는 `Gravity`의 `Show Gravity Line`, `Gravity Line Color`, `Gravity Line Opacity`, `Gravity Line Thickness`, `Gravity Line Segments`, `Lighting`의 `DynamicMeshThreshold`, `Focus`의 `Focus Zoom Multiplier`, `GenerationSeed`의 `Generation Seed`를 노출한다. `DefaultGravityLineThickness`는 사용하지 않는다. 그 외 수치 필드는 C++ 멤버로 유지하되 Details 패널에는 노출하지 않고, Data Asset에서 넘어온 spec 값을 적용하는 내부 상태로 취급한다. 에디터에 노출하지 않는 scalar/FText/enum 런타임 캐시는 `UPROPERTY` 필드로 유지하지 않고 일반 C++ 멤버로 둔다. UObject/Component 참조만 GC와 컴포넌트 관리를 위해 숨은 `UPROPERTY`로 남긴다. `GravityParent` 컴포넌트의 중력 값은 Self의 Gravity 항목 또는 Data Asset spec에서 복사되어 적용되는 내부 상태로 취급하며, 컴포넌트 자체 Details 패널에서 따로 편집하지 않는다. Surface Grid의 Face Resolution과 Planet Radius는 `CelestialBodyDynamicMesh` 반지름 기준으로 계산하며, grid cell 크기가 일정하게 유지되도록 Face Resolution을 자동 산출한다. Surface Grid는 collision trace로 표면을 다시 찾지 않고, 절차 지형과 동일한 height/normal 계산으로 grid line과 cell transform의 표면 위치를 정한다. Surface Grid의 rebuild/debug 표시 옵션은 내부 상태로 취급하고, grid line 표시는 Assembly Mode에서만 켜진다. Draw Debug Normals, Grid Subdivision, Normal Length는 Details 노출 항목으로 사용하지 않는다. `Cast Shadow Static Mesh`는 더 이상 사용하지 않는다. Static/Dynamic Mesh의 Cast Shadow, Visibility, Lighting Channels 같은 기본 옵션은 BP Editor에서 직접 설정한다. Orbit Visual 중 표시 여부, 색, 투명도, 두께는 `ASRPlanet`/`BP_Planet(Self)`의 `Orbit`에서 다루고, segment 수와 실제 궤도 운동 수치는 Data Asset 또는 generator spec에서 넘어온 내부 상태로 취급한다. `Default Surface Body Material`과 `Apply Preview in Editor` 기반 자동 preview 기능은 사용하지 않는다.
...
- `HighDetailLightingScreenSizeRatio` -> `DynamicMeshThreshold`
- `BP_SRCameraPawn`, `BP_SRPlayerController`, `BP_SolarSystemGenerator`의 C++ 부모 클래스 내부 함수/변수명은 에디터에 노출되는 Field 이름 기준으로 정리했다.
- `CastShadowStaticMesh` 컴포넌트와 관련 Shadow scale 설정은 제거했다.
- `OceanStaticMesh`, `StarPointLight`에 대한 C++ Cast Shadow 강제 설정 코드는 제거했다.
...
행성 지형 Random 생성 기능은 `FSRPlanetTerrainGenerator`와 `FSRPlanetTerrainSettings`를 중심으로 구현한다. Dynamic Mesh는 Data Asset의 `Celestial Body > Body Mesh`를 원본으로 복사한 뒤, 원본 Static Mesh의 삼각형을 사각형 셀로 복원하고 각 셀 중심 방향 기준으로 terrain sample을 계산한다. 각 셀은 원본 위치를 천체 중심 기준 scale up/down한 위치로 이동하며, 높이가 다른 인접 셀 사이는 side wall face로 연결해 틈이 생기지 않도록 한다. 표면 그리드는 Dynamic Mesh edge를 기반으로 사각형 grid line을 표시한다.

현재 코드에 구현된 `TerrainProfile`은 다음과 같다.
...
현재 terrain sample은 Minecraft Overworld의 `continentalness`, `erosion`, `peaks and valleys`, `temperature`, `humidity` 개념을 참고한 단일 sampler를 사용한다. `None`과 `GasGiant`는 procedural terrain을 생성하지 않고, `OceanWorld`는 내부 continentalness bias를 낮춰 바다 비중을 높인다. 그 외 profile은 같은 Minecraft식 sampler를 공유하며 profile별 차이는 이후 확장 대상으로 둔다. 완전한 Minecraft 원본 noise/router 구현을 그대로 이식한 것은 아니며, 현재 프로젝트의 구형 행성 mesh와 정사각형 셀 지형에 맞춘 deterministic noise 근사로 처리한다.

Data Asset의 `TerrainSettings`에서 조정하는 주요 값은 다음과 같다.

- `TerrainProfile`
- `bUseProceduralTerrain`
- `TerrainSeed`
- `TerrainHeight`
- `SeaLevel`
- `ContinentFrequency`
...
- `MicroDetailStrength`
- `MoistureFrequency`
- `TemperatureNoiseFrequency`
- `TerrainOctaves`
- `TerrainPersistence`

`FSRPlanetTerrainSample`은 `HeightOffset`, `Continent`, `MountainMask`, `Temperature`, `Moisture`, `RiverMask`, `LakeMask`, `CraterMask`, `PlateBeltMask`, `Biome`, `SurfaceColor`를 반환한다. EarthLike 계열의 `SurfaceColor`는 높이 명암보다 biome 색상을 우선해서 지구형으로 읽히도록 조정하며, Snow/Ice는 낮은 온도만으로 넓게 칠하지 않고 극지 육지 또는 고산 조건을 함께 요구한다. Dynamic Mesh는 `SurfaceColor`를 vertex color로 기록하므로, `Celestial Body > Material`에 지정한 Material이 Vertex Color를 Base Color 또는 색상 블렌딩에 사용해야 terrain 색상이 화면에 반영된다.

현재 C++ 기본값 기준으로 Planet Data Asset은 `EarthLike`, Moon Data Asset은 `RockyMoon`을 사용한다. 기존에 저장된 `.uasset` Data Asset은 C++ 생성자 기본값이 자동으로 덮이지 않을 수 있으므로 에디터에서 `TerrainSettings` 값을 직접 확인해야 한다.

주요 에디터 점검 대상은 다음과 같다.
...
## 9. Blueprint Option Rule

- Blueprint/Details 패널에서 직접 설정 가능한 Component 옵션은 C++에서 임의로 강제 변경하지 않는다.
- 예: Collision, Cast Shadow, Visibility, Lighting Channels 같은 런타임 확인이 필요한 옵션은 코드로 덮어쓰지 말고 사용자가 BP/DA에서 설정할 항목을 명확히 알려준다.
- 런타임 로직상 반드시 코드에서 강제해야 하는 값만 예외로 두며, 그 경우 이유와 영향 범위를 README와 작업 응답에 기록한다.
