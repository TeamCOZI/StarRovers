...
현재 C++ 기반 공통 Actor는 `ASRCelestialBody`다. 이전 이름은 `AProceduralCelestialBody`였으며, Blueprint 에셋은 `BP_CelestialBodyType`에서 `BP_CelestialBody`로 정리하는 방향이다.

`BP_CelestialBody`에 남길 공통 컴포넌트는 다음과 같다.

- `CelestialBodyStaticMesh`: 천체의 기본 시각 메시
...
`BP_Star`는 `ASRStar`를 부모로 두고, 항성 전용 동작을 담당한다. `ASRStar`는 `ASRCelestialBody`를 상속하며 Native `StarPointLight`와 항성 표면 flicker 파라미터 업데이트를 담당한다. Star flicker 수치는 `BP_Star`의 Details에 노출하지 않고 Star Data Asset의 `Star` 항목에서 받은 spec 값으로 적용한다. 기존 `BP_Star`에 SCS 컴포넌트로 저장된 `StarPointLight`가 남아 있으면 부모를 `ASRStar`로 바꾼 뒤 에디터에서 중복 컴포넌트를 정리해야 한다.

항성 이글거림은 C++ 지오메트리 변형이 아니라 Star 머티리얼 Scalar Parameter(`StarFlickerTime`, `StarFlicker`, `StarFlickerStrength`, `StarEmissiveFlicker` 또는 `StarTime`, `FlickerAlpha`, `EmissivePulse`)로 구동한다. `ASRStar`는 시간, 노이즈 기반 flicker 값, 약한 Point Light 강도 변화를 공급하고, 실제 표면 노이즈/발광 애니메이션은 머티리얼이 담당한다.
...
Star Data Asset의 `Star Rovers` 항목에는 `Identity`, `Celestial Body`, `Star`, `Gravity`만 노출한다. `Identity`에는 `Display Name`과 `Celestial Body Category`, `Celestial Body`에는 `Scale`, `Body Mesh`, `Material`, `Star`에는 `Star Material Emissive Strength`, `Star Point Light Intensity`, `Star Point Light Color`, `Star Flicker Speed`, `Star Flicker Strength`, `Star Flicker Noise Scale`, `Star Light Flicker Strength`, `Gravity`에는 `Mass`, `Gravity Ratio`, `Gravity Radius Ratio`를 둔다. Star DA의 runtime biome spec은 이 전용 필드에서 생성한다.
...
C++ 클래스/프로퍼티 리다이렉트는 `Config/DefaultEngine.ini`에 추가해서 기존 Blueprint 참조가 새 이름으로 이전될 수 있게 한다.
...
`ASRPlanet`의 Details 노출은 `Surface`의 `Construction Height Offset`, `Grid Line Color`, `Grid Line Opacity`, `Grid Line Thickness`, `Hovered Cell Color`, `Selected Cell Color`, `Occupied Cell Color`와 `Orbit`의 `Show Orbit Line`, `Orbit Line Color`, `Orbit Line Opacity`, `Orbit Line Thickness`로 제한한다. `USROrbit` 컴포넌트의 Orbit Visual UPROPERTY는 Details 편집 대상에서 제외하고, `ASRCelestialBody`가 `ConfigureOrbitPathVisual()`로 Self/Data Asset에서 정리된 값을 전달한다. `USRPlanetSurfaceGrid` 컴포넌트의 Face Resolution, Planet Radius, Rebuild Grid On Register, Debug Grid 표시 옵션, Draw Debug Normals, Grid Subdivision, Normal Length, Terrain 수치는 Details 편집 대상에서 제외한다. Shadow Scale, Orbit Speed, Orbit Line Segments는 `FSRCelestialBodyBiomeSpec`에 포함되어 Planet/Moon Data Asset에서 조정한다. `USRPlanetDataAsset`과 `USRMoonDataAsset`은 동일한 노출 항목을 사용하며, 생성기는 두 타입 모두 `BuildBiomeSpec()`와 Scale/Mass/Gravity 값을 통해 spec을 구성한다.

### 5.3 현재 BP/DA 에셋 상태

현재 파일 기준으로 확인된 Blueprint Class 에셋은 다음과 같다.

- `Content/BlueprintClasses/Camera/BP_StarMapCameraPawn.uasset`
- `Content/BlueprintClasses/Camera/BP_StarMapGameMode.uasset`
- `Content/BlueprintClasses/Camera/BP_StarMapPlayerController.uasset`
- `Content/BlueprintClasses/Celestial/BP_CelestialBody.uasset`
- `Content/BlueprintClasses/Celestial/BP_Planet.uasset`
- `Content/BlueprintClasses/Celestial/BP_SpaceSkySphere.uasset`
- `Content/BlueprintClasses/Celestial/BP_Star.uasset`
- `Content/BlueprintClasses/Core/BP_SolarSystemGenerator.uasset`
- `Content/BlueprintClasses/Core/BP_SolarSystemManager.uasset`

현재 파일 기준으로 확인된 Data Asset 에셋은 다음과 같다.

- `Content/BlueprintClasses/Celestial/DA_StarPreset_Basic.uasset`
- `Content/BlueprintClasses/Celestial/DA_PlanetPreset_Basic.uasset`
- `Content/BlueprintClasses/Celestial/DA_MoonPreset_Basic.uasset`

위 BP/DA의 실제 부모 클래스와 Data Asset 타입은 에디터에서 확인해야 한다. 최근 C++ 이름 변경은 `Config/DefaultEngine.ini` 리다이렉트로 이어지도록 정리했지만, `.uasset` 내부 저장 상태와 SCS 컴포넌트 중복 여부는 에디터에서 직접 확인한다.
...
Blueprint와 Data Asset 에셋은 예를 들어 `BP_CelestialBody`, `BP_Star`, `DA_PlanetPreset_Basic`처럼 쓴다.
