...
{
	inline double GetSolarSystemGenerationTimingSeconds()
	{
		return FPlatformTime::Seconds();
	}

	inline double GetSolarSystemGenerationElapsedMilliseconds(double StartSeconds)
	{
		return (FPlatformTime::Seconds() - StartSeconds) * 1000.0;
	}

	struct FSRSolarSystemOrbitLayoutEntry
	{
		float OrbitingBodyClearanceRadius = 0.0f;
		float DesiredOrbitRadius = 0.0f;
	};

	struct FSRSolarSystemGenerationStageTiming
	{
		FString Name;
		...
		FSRCelestialBodyData& OutData)
	{
		...
		OutData.InitialStellarFuelDecreasePerSecond = FMath::Max(0.0, DataAssetData.InitialStellarFuelDecreasePerSecond);
	...
	inline void ApplyClassDefaultRuntimeDisplaySettings(
		const TSubclassOf<ASRCelestialBody>& BodyClass,
		...
		FSRCelestialBodyGenerateRequest& OutRequest)
	{
		...
		ApplyClassDefaultRuntimeDisplaySettings(BodyClass, OutRequest.BodyData);
