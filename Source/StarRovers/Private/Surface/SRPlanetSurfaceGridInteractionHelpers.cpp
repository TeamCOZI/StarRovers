...
{
	FSRSurfaceGridDisplayMapper::FSRSurfaceGridDisplayMapper(int32 InFaceResolution)
		: SafeFaceResolution(FMath::Max(1, InFaceResolution))
	{
	}

	FSRSurfaceGridDisplayCoord FSRSurfaceGridDisplayMapper::CanonicalToDisplay(const FSRPlanetSurfaceGridCellId& CellId) const
	{
		FSRSurfaceGridDisplayCoord Coord;
	...
	FSRPlanetSurfaceGridCellId FSRSurfaceGridDisplayMapper::DisplayToCanonical(const FSRSurfaceGridDisplayCoord& Coord) const
	{
	...
	bool FSRSurfaceGridDisplayMapper::TryRotateWalkBasisAcrossEdge(
	...
	bool FSRSurfaceGridDisplayMapper::TryStepDisplayCoord(
	...
	FSRSurfaceGridDisplayFaceBasis FSRSurfaceGridDisplayMapper::GetDisplayFaceBasis(ESRCubeSphereFace Face) const
	{
		const FSRSurfaceGridCubeFaceBasis CubeBasis = GetCubeFaceBasis(Face);
		FSRSurfaceGridDisplayFaceBasis Basis;
	...
	FVector FSRSurfaceGridDisplayMapper::ResolveDisplayDirection(
		ESRCubeSphereFace Face,
		const FIntPoint& LocalDirection) const
	{
		const FSRSurfaceGridDisplayFaceBasis Basis = GetDisplayFaceBasis(Face);
		return (Basis.AxisX * static_cast<float>(LocalDirection.X))
			+ (Basis.AxisY * static_cast<float>(LocalDirection.Y));
	}

	bool FSRSurfaceGridDisplayMapper::TryResolveLocalDisplayDirection(
		ESRCubeSphereFace Face,
	...
	FVector FSRSurfaceGridDisplayMapper::RotateDisplayDirectionAcrossEdge(
		const FVector& SourceWorldDirection,
		ESRCubeSphereFace SourceFace,
		ESRCubeSphereFace TargetFace) const
	{
		const FSRSurfaceGridDisplayFaceBasis SourceBasis = GetDisplayFaceBasis(SourceFace);
		const FSRSurfaceGridDisplayFaceBasis TargetBasis = GetDisplayFaceBasis(TargetFace);
	...
	bool FSRSurfaceGridDisplayMapper::TryTransitionDisplayEdge(
	...
	bool FSRSurfaceGridInteractionPatchBuilder::BuildPatchCellIds(
		...
		TArray<FSRPlanetSurfaceGridCellId>& OutCellIds)
	{
		...
		const FSRSurfaceGridDisplayMapper DisplayMapper(SafeFaceResolution);

		auto TryStepWalker = [&DisplayMapper, &IsValidCell](
			FSRSurfaceGridDisplayCoord& CurrentCoord,
			FSRSurfaceGridDisplayWalkBasis& WalkBasis,
			bool bIsGlobalX,
			int32 StepSign)
		{
			...
			FSRSurfaceGridDisplayCoord NextCoord;
		...
		{
			FSRSurfaceGridDisplayCoord CurrentCoord = DisplayMapper.CanonicalToDisplay(CenterCellId);
			FSRSurfaceGridDisplayWalkBasis WalkBasis;
		...
		for (int32 CellY = ClippedMinY; CellY <= ClippedMaxY; ++CellY)
		{
			...
			{
				FSRSurfaceGridDisplayCoord PatchCoord = CenterDisplayCoord;
				PatchCoord.X = CellX;
				PatchCoord.Y = CellY;
