...
namespace
{
	...
	int32 GetFaceVertexIndex(int32 VertexX, int32 VertexY, int32 VertexResolution)
	{
		return (VertexY * VertexResolution) + VertexX;
	}

	void BuildConformalFaceDirections(ESRCubeSphereFace Face, int32 Resolution, TArray<FVector>& OutDirections)
	{
		OutDirections.Reset();
		if (Resolution <= 0)
		{
		...
		OutDirections.SetNum(VertexResolution * VertexResolution);
		...
		for (int32 VertexY = 0; VertexY < VertexResolution; ++VertexY)
		{
			...
			for (int32 VertexX = 0; VertexX < VertexResolution; ++VertexX)
			{
				...
				OutDirections[GetFaceVertexIndex(VertexX, VertexY, VertexResolution)] =
	...
	bool BuildConformalCubeCell(
		...
		const TArray<FVector>& FaceDirections,
		FSRPlanetSurfaceGridCell& OutCell)
	{
		...
		const int32 VertexResolution = Resolution + 1;
		const int32 Corner00Index = GetFaceVertexIndex(CellId.CellX, CellId.CellY, VertexResolution);
		const int32 Corner10Index = GetFaceVertexIndex(CellId.CellX + 1, CellId.CellY, VertexResolution);
		const int32 Corner11Index = GetFaceVertexIndex(CellId.CellX + 1, CellId.CellY + 1, VertexResolution);
		const int32 Corner01Index = GetFaceVertexIndex(CellId.CellX, CellId.CellY + 1, VertexResolution);
		if (!FaceDirections.IsValidIndex(Corner00Index)
			|| !FaceDirections.IsValidIndex(Corner10Index)
			|| !FaceDirections.IsValidIndex(Corner11Index)
			|| !FaceDirections.IsValidIndex(Corner01Index))
		{
			return false;
		}
		...
		OutCell.Corner00 = FaceDirections[Corner00Index] * Radius;
		OutCell.Corner10 = FaceDirections[Corner10Index] * Radius;
		OutCell.Corner11 = FaceDirections[Corner11Index] * Radius;
		OutCell.Corner01 = FaceDirections[Corner01Index] * Radius;
		OutCell.LocalCenter = (FaceDirections[Corner00Index] + FaceDirections[Corner10Index] + FaceDirections[Corner11Index] + FaceDirections[Corner01Index]).GetSafeNormal() * Radius;
...
{
	TArray<FVector> FaceDirections;
	BuildConformalFaceDirections(CellId.Face, Resolution, FaceDirections);
	return BuildConformalCubeCell(Resolution, Radius, CellId, FaceDirections, OutCell);
}
...
TArray<FSRPlanetSurfaceGridCell> USRPlanetSurfaceGridLibrary::GenerateCubeSphereCells(int32 Resolution, float Radius)
{
	...
	for (uint8 FaceValue = static_cast<uint8>(ESRCubeSphereFace::PositiveX); FaceValue <= static_cast<uint8>(ESRCubeSphereFace::NegativeZ); ++FaceValue)
	{
		...
		TArray<FVector> ConformalFaceDirections;
		BuildConformalFaceDirections(Face, Resolution, ConformalFaceDirections);
		...
		for (int32 CellY = 0; CellY < Resolution; ++CellY)
		{
			for (int32 CellX = 0; CellX < Resolution; ++CellX)
			{
				...
				if (BuildConformalCubeCell(Resolution, Radius, CellId, ConformalFaceDirections, Cell))
				{
					Cells.Add(Cell);
				}
			}
