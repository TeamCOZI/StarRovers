...
void StarRovers::Conveyor::FSRConveyorDebugLineRenderer::Draw(
	...
	const FSRConveyorDebugLineSettings& Settings)
{
	if (!IsValid(SurfaceGrid)
		|| !IsValid(LineBatchComponent)
		|| (!Settings.bShowPathDebugLine && !Settings.bShowConnectionDebugLine))
	{
		return;
	}

	const FVector SurfaceCenter = SurfaceGrid->GetComponentTransform().GetLocation();
	if (Settings.bShowPathDebugLine)
	{
		const FColor LineColor = Settings.PathDebugLineColor.ToFColor(true);
		for (const FSRConveyorBeltPath& BeltPath : BeltPaths)
		{
			...
			if (!SurfaceGrid->GetCellInfoById(BeltPath.CellIds[0], StartCellInfo)
				|| !SurfaceGrid->GetCellInfoById(BeltPath.CellIds.Last(), EndCellInfo))
			{
				continue;
			}

			const float HeightOffset = static_cast<float>(FMath::Max(0, BeltPath.Layer)) * FMath::Max(0.0f, BeltPath.LayerHeight)
				+ Settings.BeltSurfaceOffset
			...
			DrawDebugLine(LineBatchComponent, StartPoint, EndPoint, LineColor, Settings.PathDebugLineThickness * ConveyorDebugLineSizeScale);
		}
	}

	if (!Settings.bShowConnectionDebugLine || Segments.IsEmpty())
	{
		return;
	}

	const FColor ConnectionColor = Settings.ConnectionDebugLineColor.ToFColor(true);
	const FColor BrokenConnectionColor = Settings.BrokenConnectionDebugLineColor.ToFColor(true);
	const FColor EndpointColor = Settings.EndpointDebugLineColor.ToFColor(true);
	const float LineThickness = FMath::Max(0.0f, Settings.ConnectionDebugLineThickness * ConveyorDebugLineSizeScale);
	for (const TPair<FSRConveyorLaneKey, FSRConveyorSegment>& SegmentPair : Segments)
	{
		...
		const float HeightOffset =
			static_cast<float>(FMath::Max(0, Segment.Lane.Layer)) * Settings.ConnectionLayerHeight
			+ Settings.BeltSurfaceOffset
			+ Settings.ConnectionDebugLineHeightOffset * ConveyorDebugLineSizeScale;
		...
		{
			DrawDebugCross(LineBatchComponent, SegmentPoint, SegmentNormal, 120.0f * ConveyorDebugLineSizeScale, EndpointColor, LineThickness);
			continue;
		}
		...
		for (const ESRConveyorGridDirection OutputDirection : OutputDirections)
		{
			...
			if (!FSRConveyorConnectionQuery::TryResolveLaneByDirection(SurfaceGrid, Segment, OutputDirection, NextLaneKey))
			{
				DrawDebugCross(LineBatchComponent, SegmentPoint, SegmentNormal, 120.0f * ConveyorDebugLineSizeScale, BrokenConnectionColor, LineThickness);
				continue;
			}
